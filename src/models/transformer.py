...
class AttentionBlock(eqx.Module):
    ...
    def __init__(self, hidden_size: int, num_heads: int, dropout_rate: float, key: PRNGKeyArray):
        self.attention = eqx.nn.MultiheadAttention(
            num_heads, hidden_size, hidden_size, hidden_size, hidden_size, key=key
        )
        self.layernorm = eqx.nn.LayerNorm(shape=(hidden_size,))
        self.dropout = eqx.nn.Dropout(dropout_rate)
    ...
    def __call__(
        ...
        keys = jrandom.split(key)
        # Arg 'inputs' can be a tuple of three arrays for cross attention, or a single array for self attention.
...
class FeedForwardBlock(eqx.Module):
    ...
    def __init__(
        self, hidden_size: int, intermediate_size: int, dropout_rate: float, key: PRNGKeyArray
    ):
        keys = jrandom.split(key)
...
class SelfAttnEncoder(eqx.Module):
    ...
    def __call__(
        self, dynamic_data: Array, static_encoded: Array, mask: Array | None, key: PRNGKeyArray
    ) -> Array:

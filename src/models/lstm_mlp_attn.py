    ...
    def __init__(self, *, 
        ...
        if len(cross_vars) > 0:
            for var_name, var_key in zip(cross_vars, decoder_keys):
                self.decoders[var_name] = CrossAttnDecoder(seq_length, hidden_size, hidden_size, num_layers, num_heads, dropout, entity_aware, var_key)
        else:
            self.decoders['self'] = CrossAttnDecoder(seq_length, hidden_size, hidden_size, num_layers, num_heads, dropout, entity_aware, var_key)
